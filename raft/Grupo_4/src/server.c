...
#define SIZE_ENTRIES		8
#define TIMER_SERVER_START	500
                    ...
                    void **result)
{
	...
	printf("Server %d : Received data succesfully\n", id);
...
static void serverTimerCb(uv_timer_t *timer)
{
	...
	struct Fsm fsm = {0};
	memcpy(&fsm, catalog, sizeof(struct Fsm));
	
	if (poll(&mypoll, 1, 0)){
		scanf("%s %s", input.key, input.value);

		for (int i =0 ; i < SIZE_ENTRIES; i++){
			if(strcmp(fsm.entries[i].key, input.key) == 0){//si la clave es correcta se accede a modificar el valor
				if(strcmp("reset", input.value) == 0){
					memset(&fsm.entries[i], '\0', sizeof(pair_t));//si se introduce reset se borran las clave-valor almacenadas en esa posicion
				}
				
				else{
					strcpy(fsm.entries[i].value, input.value);//se copia el nuevo valor
				}
			}
			
			else if((fsm.entries[i].key[0] == '\0') && (strcmp("/", input.value) != 0)){
				strcpy(fsm.entries[i].key, input.key);
				strcpy(fsm.entries[i].value, input.value);//si no habia ninguna clave almacenada se guardan las entradas como nuevas
				break;
			}
		}	
	}
...
static int ServerStart(struct Server *s)
{
    ...
    rv = uv_timer_start(&s->timer, serverTimerCb, 0, TIMER_SERVER_START);
    if (rv != 0) {

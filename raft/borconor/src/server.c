...
static int FsmApply(struct raft_fsm *fsm,
                    ...
                    void **result)
{
    struct Fsm *f = fsm->data, *fprueba = buf->base;
    if (buf->len != sizeof(struct Fsm)) {
        return RAFT_MALFORMED;
    }
	printf("Server %d : recibido\n", id);

    memcpy(f->entries, buf->base, sizeof(struct Fsm));// f->count += *(uint64_t *)buf->base;
    *result = &f->entries;
    return 0;
}
                       ...
                       unsigned *n_bufs)
{
    ...
    *bufs = raft_malloc(sizeof **bufs);
    ...
    (*bufs)[0].len = sizeof(struct Fsm);//sizeof(uint64_t);
    (*bufs)[0].base = raft_malloc((*bufs)[0].len);
    if ((*bufs)[0].base == NULL) {
        return RAFT_NOMEM;
    }
    
    memcpy(bufs[0]->base, f->entries, sizeof(struct Fsm)); //*(uint64_t *)(*bufs)[0].base = f->count;
    return 0;
}
...
{
    struct Fsm *f = raft_malloc(sizeof (struct Fsm));
    if (f == NULL) {
        return RAFT_NOMEM;
    }
    
    memset(f, NULL, sizeof(struct Fsm));
    tabla = f;
                      ...
                      unsigned id)
{
    ...
    raft_set_snapshot_threshold(&s->raft, 4);
    raft_set_snapshot_trailing(&s->raft, 2);
...
static void serverApplyCb(struct raft_apply *req, int status, void *result)
{
    ...
    memcpy(&f, result, sizeof(struct Fsm));
	for (int i = 0 ; i < (sizeof(struct Fsm)/sizeof(pair_t)); i++){
		Logf(s->id, "key %s, value %s", f.entries[i].key, f.entries[i].value);
	}
   // Logf(s->id, "key %s", f.entries[0].key);
}
...
static void serverTimerCb(uv_timer_t *timer)
{
	...
	struct Fsm fsm = {0};
	memcpy(&fsm, tabla, sizeof(struct Fsm));
	
	if (poll(&mypoll, 1, 0)){
		scanf("%s %s", vk.key, vk.value);

		//if !(memcmp( cmd, "SET", sizeof("SET")))
		for (int i =0 ; i < (sizeof(struct Fsm)/sizeof(pair_t)); i++){
			if(strcmp(fsm.entries[i].key, vk.key) == 0){
				if(strcmp("/", vk.value) == 0){
					memset(&fsm.entries[i], '\0', sizeof(pair_t));
				}
				
				else{
					strcpy(fsm.entries[i].value, vk.value);
				}
			}
			
			else if((fsm.entries[i].key[0] == '\0') && (strcmp("/", vk.value) != 0)){
				strcpy(fsm.entries[i].key, vk.key);
				strcpy(fsm.entries[i].value, vk.value);
				break;
			}
		}	
    ...
    memcpy(buf.base, &fsm, buf.len);
  
   // Logf(s->id, "enviando:  %s, %s", entries.key, entries.value);
